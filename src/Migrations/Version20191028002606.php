 ...
 */
final class Version20191028002606 extends AbstractMigration
{
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, password, email, roles FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, roles CLOB DEFAULT \'ROLE_USER\' NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, username, password, email, roles) SELECT id, username, password, email, roles FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
